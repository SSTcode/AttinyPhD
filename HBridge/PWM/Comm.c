...
uint16_t MdbCrc (char *nData, uint8_t wLength)
{
	...
	register uint16_t wCRCWord = 0xFFFF;

	while (wLength--)
	{
		register uint8_t nTemp = *nData++ ^ wCRCWord;
		wCRCWord = wCRCTable[nTemp] ^ (wCRCWord >> 8);
	}
	return wCRCWord;
}
...
void IO_update(void){
	GPIO_WRITE(0 + 3, Outputs.PWM_Supply_pos);//Port A pin 5, Outpu.bit_field
	GPIO_WRITE(0 + 2, Outputs.PWM_Supply_neg);//Port A pin 7
...
void IO_init(void){
	*(uint8_t *)&Outputs = 0;
	...
	GPIO_DIR(0 + 3, 0); /* set pin 7 of PORT A as output */
	GPIO_DIR(0 + 2, 0); /* set pin 5 of PORT A as output */
...
ISR(USART0_DRE_vect){
	...
	if(++data_count_tx == DATA_TX_LENGTH + 2) USART0.CTRLA &= ~USART_DREIE_bm;
}
